...
class HFHighLevelPlanner:
    def __init__(self, model_path: str):
        self.model = AutoModelForCausalLM.from_pretrained(model_path)
        self.tokenizer = AutoTokenizer.from_pretrained(model_path)
    
    def get_subgoal_instruction(self, obs: dict) -> str:
        return None
...
if __name__ == "__main__":
    ...
    for i in trange(args.num_episodes):
        ...
        while not done:
            ...
            if step % args.subgoal_interval == 0:
                if step == 0:
                    prev_image = obs['video.right_view'].squeeze(0)
                    prev_subgoal = "No previous subgoal provided yet."
                think, subgoal_instruction = planner.get_subgoal_instruction_curr_multiview(obs, prev_subgoal)
                print("--------------------------------")
                print(f"Think: {think}")
                print(f"Subgoal instruction: {subgoal_instruction}")
                print("--------------------------------")
                # update prev image and subgoal
                prev_subgoal = subgoal_instruction
                prev_image = obs['video.right_view'].squeeze(0)
                
                episode_subgoal_info.append(
                    (subgoal_instruction, 
                     [args.subgoal_interval * (step // args.subgoal_interval),
                    args.subgoal_interval * (step // args.subgoal_interval + 1)]
                ))
            #################################################################################    
            # print("=== OBS SUMMARY ===")
