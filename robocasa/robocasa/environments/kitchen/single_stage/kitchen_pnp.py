...
class PnPCabToSink(PnP):
    """
    Class encapsulating the atomic cabinet to sink pick and place task.
    IMPORTANT: Requires sink with open cabinet directly above it.
    Currently fixed to layout 0 (ONE_WALL_SMALL) only.
    ...
    Args:
        ...
        layout_ids (list): Override to use specific layouts. Default: [0] (ONE_WALL_SMALL)
    """

    def __init__(
        self, 
        cab_id=FixtureType.CABINET_TOP, 
        obj_groups="all",
        layout_ids=None,
        ...
        **kwargs
        ...
        # FORCE layout 0 (ONE_WALL_SMALL) - ignore any external layout specifications
        # This task requires sink with cabinet above it, which is guaranteed in layout 0
        kwargs['layout_and_style_ids'] = [(0, s) for s in range(12)]
        
        # Remove conflicting layout parameters from kwargs
    ...
    def _setup_kitchen_references(self):
        """
        Setup the kitchen references for the cabinet to sink pick and place task:
        The sink (destination, robot spawn point) and cabinet above it (source)
        """
        super()._setup_kitchen_references()
        
        # Register sink first (destination and robot spawn point)
        self.sink = self.register_fixture_ref(
            "sink",
            dict(id=FixtureType.SINK),
        )
        
        # Register cabinet near/above the sink
        # The ref parameter ensures it picks a cabinet close to the sink
        self.cab = self.register_fixture_ref(
            "cab",
            dict(id=self.cab_id, ref=self.sink),
        )
        
        # Robot spawns in front of SINK (not cabinet)
        self.init_robot_base_pos = self.sink

    def get_ep_meta(self):
        """
        Get the episode metadata for the cabinet to sink pick and place task.
        This includes the language description of the task.
        """
        ep_meta = super().get_ep_meta()
        obj_lang = self.get_obj_lang()
        ep_meta[
            "lang"
        ] = f"pick the {obj_lang} from the cabinet and place it in the sink"
        return ep_meta

    def _reset_internal(self):
        """
        Resets simulation internal configurations.
        Ensures ONLY the cabinet above the sink is fully open.
        """
        super()._reset_internal()
        
        # STEP 1: Close ALL cabinet doors to ensure clean state
        for fixture_name, fixture in self.fixtures.items():
            if 'cab' in fixture_name.lower() and hasattr(fixture, 'set_door_state'):
                try:
                    fixture.set_door_state(min=0.0, max=0.0, env=self, rng=self.rng)
                except:
                    pass  # Some cabinets might not have doors
        
        # STEP 2: Open ONLY the cabinet near the sink (self.cab)
        # min=1.0, max=1.0 ensures deterministic 100% open (not 90-100% random)
        try:
            self.cab.set_door_state(min=1.0, max=1.0, env=self, rng=self.rng)
        except Exception as e:
            print(f"Warning: Could not open cabinet {self.cab.name}: {e}")

    def _get_obj_cfgs(self):
        """
        Get the object configurations for the cabinet to sink pick and place task.
        Puts the target object in the front area of the cabinet above the sink.
        Puts distractor objects in the back of the cabinet and in the sink.
        """
        cfgs = []
        # Target object in cabinet (left side, front)
        cfgs.append(
            dict(
                name="obj",
                obj_groups=self.obj_groups,
                ...
                placement=dict(
                    fixture=self.cab,
                    size=(0.40, 0.20),
                    pos=(-0.5, 0.0),  # Left side of cabinet
                ),
            )
        )

        # Distractor in cabinet (right side)
        cfgs.append(
            dict(
                name="distr_cab",
                obj_groups="all",
                placement=dict(
                    fixture=self.cab,
                    size=(0.40, 0.20),
                    pos=(0.5, 0.0),  # Right side of cabinet
                    offset=(0.0, 0.0),
                ),
            )
        )
        
        # Distractor in sink (left or right side)
        cfgs.append(
            dict(
                name="distr_sink",
                obj_groups="all",
    ...
    def _check_success(self):
        """
        Check if the cabinet to sink pick and place task is successful.
        Checks if the object is inside the sink and the gripper is far from the object.
...
class PnPSinkToCab(PnP):
    """
    Class encapsulating the atomic sink to cabinet pick and place task.
    IMPORTANT: Requires sink with open cabinet directly above it.
    Currently fixed to layout 0 (ONE_WALL_SMALL) only.

    Args:
        cab_id (str): The cabinet fixture id to place the object in.
    ...
    def __init__(
        ...
        **kwargs
        ...
        # FORCE layout 0 (ONE_WALL_SMALL) - ignore any external layout specifications
        # This task requires sink with cabinet above it, which is guaranteed in layout 0
        kwargs['layout_and_style_ids'] = [(0, 0), (0, 1), (0, 2)]
    ...
    def _setup_kitchen_references(self):
        """
        Setup the kitchen references for the sink to cabinet pick and place task:
        The sink (source, robot spawn point) and cabinet above it (destination)
        """
        super()._setup_kitchen_references()
        
        # Register sink first (source and robot spawn point)
        self.sink = self.register_fixture_ref(
            "sink",
            dict(id=FixtureType.SINK),
        )
    ...
    def get_ep_meta(self):
        """
        Get the episode metadata for the sink to cabinet pick and place task.
        This includes the language description of the task.
        """
        ep_meta = super().get_ep_meta()
        obj_lang = self.get_obj_lang()
        ep_meta[
            "lang"
        ] = f"pick the {obj_lang} from the sink and place it in the cabinet"
        return ep_meta
    ...
    def _get_obj_cfgs(self):
        """
        Get the object configurations for the sink to cabinet pick and place task.
        Puts the target object in the sink. Puts distractor objects in the back 
        of the sink and back area of the cabinet above.
        """
        cfgs = []
        # Target object in sink (left side)
        cfgs.append(
            dict(
                name="obj",
                obj_groups=self.obj_groups,
        ...
        cfgs.append(
            dict(
                ...
                placement=dict(
                    fixture=self.sink,
                    size=(0.30, 0.30),
                    pos=(1.0, 0.0),  # Right side of sink
                ),
            )
        )
        ...
        cfgs.append(
            dict(
                ...
                placement=dict(
                    fixture=self.cab,
                    size=(0.40, 0.20),
                    pos=(1.0, 0.0),  # Right side of cabinet
                    offset=(0.0, 0.0),
                ),
            )
        )
    ...
    def _check_success(self):
        """
        Check if the sink to cabinet pick and place task is successful.
        Checks if the object is inside the cabinet and the gripper is far from the object.
